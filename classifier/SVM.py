...
from elm import GenELMClassifier, ELMClassifier
...
data = np.loadtxt("work/Ce/fcgr-1-2-4.csv", dtype=np.float, delimiter=',')
...
for train_index, test_index in sKF.split(std_X,y):
    ...
    y_test = y[test_index]
    #-------------------------------------------------------------------------------,subsample=0.8
    #clf = MLPClassifier(solver='adam', alpha=3e-3,hidden_layer_sizes=(100,100,100), early_stopping=True,batch_size=512) 
    #clf = xgb.XGBClassifier(max_depth=3,learning_rate=0.1, n_estimators=200, gamma=6,subsample=0.8,reg_lambda=0,objective='binary:logistic',colsample_bytree=0.8)
    clf = SVC(C=1.5, kernel='rbf',gamma=0.3,decision_function_shape='ovr',probability=True)  #gamma=0.65,
    ...
    print('train_Accuracy: {:.4f}'.format(clf.score(x_train, y_train)))
